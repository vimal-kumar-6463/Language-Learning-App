...
function WritingPage(props) {
    ...
    useEffect(() => {
        (async() => {
            ...
            .then(res => {
                setProgress(sum(res.data.writing));
                console.log("Progress : " , sum(res.data.writing))
            })
    ...
    return (
        <View style={styles.container}>
            <View style={styles.topBox}>
                <Text style={styles.topText}>{"Writing"}</Text>
                <Image style={styles.topImage} source={require("../assets/backPack.png")}/>
            </View>
            <View style={styles.taskList}>
                {syllabus.map((item , index) =>                 
                    <Pressable style={styles.taskItem} android_ripple={{color:"#2c3a4d"}} key={index} onPress={(progress >= (index) * 20)?() => props.navigation.navigate("ModulePage",{navigateTo : "WritePracticePage"}):()=>{}} >
